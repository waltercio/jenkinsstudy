 ...
 *  Based on: QX-3374/QX-2477
 */

class AiAlertWorklogMs extends BaseTest {

  // Name of each request
  val req1 = "GET - Calling ai-alert-ms to get alertId and alertCreateTimeInSeconds"
  val req2 = "POST - Add worklog into alertId"
  val req3 = "GET - Validate if worklog was added correctly"
  ...
  val scn = scenario("AiAlertWorklogMs")
    ...
    .exec(http(req2)
      .post("micro/ai_alert_worklog")
      .basicAuth(adUser, adPass)
      .body(StringBody("{ \"alertId\": ${alertId}, \"alertCreateTimeInSeconds\": ${alertCreateTimeInSeconds}, \"logText\": \"Test worklog microservice\", \"submitter\": \"QA Automation\", \"logType\": \"AI_SYSTEM\", \"visibility\": \"SOC\"}"))
      .check(status.is(204))
    ...
    .exec(http(req3)
      ...
      .check(jsonPath("$..logText").exists)
      .check(jsonPath("$..logDate").exists)
      .check(jsonPath("$..logTime").exists)
      .check(jsonPath("$..submitter").exists)
      .check(jsonPath("$..logType").exists)
      .check(jsonPath("$..visibility").is("SOC"))
      .check(jsonPath("$..content[-1:]..logText").is("Test worklog microservice"))
      .check(jsonPath("$..content[-1:]..submitter").is("QA Automation"))
    ...
    .exec(http(req4)
      ...
      .basicAuth(adUser, adPass)
      .check(status.is(400))
      .check(jsonPath("$..errors..alertCreateTimeInSeconds").is("[\"you must provide a alertCreateTimeInSeconds\"]"))
