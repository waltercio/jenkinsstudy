 ...
 * Automation task for this script: https://jira.sec.ibm.com/browse/QX-11338
 * Functional test link: https://jira.sec.ibm.com/browse/QX-11336
 */


class AtdsPredictionMs extends BaseTest {

  //local variable
  val atdsPrediction = "micro/atds-prediction/predict?explainable=false"

  //payload paths
  val payload = "/tests/resources/atds_prediction_ms/atds_prediction_ms_payload.json"
  val payloadNegativeTest = "/tests/resources/atds_prediction_ms/atds_prediction_ms_payload_negative_test.json"

  //  Name of each request
  val req01 = "Post ATDS Prediction data"
  ...
  val scn = scenario("AtdsPredictionMs")

    //Post ATDS Prediction data
    .exec(http(req01)
      .post(atdsPrediction)
      .header("uuidATDS", "test123")
      .body(RawFileBody(currentDirectory + payload)).asJson
      .check(status.is(200))
      .check(jsonPath("$..alert_id").exists)
