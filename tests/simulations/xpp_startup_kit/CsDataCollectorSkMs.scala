  ...
  *  Automation task for this script: https://jira.sec.ibm.com/browse/QX-11042
  *  Functional test link: https://jira.sec.ibm.com/browse/QX-11030
  */


 class CsDataCollectorSkMs extends Simulation {
   ...
   val configurations = JsonMethods.parse(Source.fromFile(
     currentDirectory + "/tests/resources/xpp_startup_kit/cs_data_collector_configuration.json").getLines().mkString)
 ...
 val req7 = "Metrics: Verify the amount of success metrics increased after posting a new message"
   ...
   val httpProtocolCsDataCollectorSkMs = http
     .baseUrl(baseUrl)
     .header("Content-Type", "application/json")

   val scn = scenario("CsDataCollectorSkMs")
   ...
   ).protocols(httpProtocolCsDataCollectorSkMs).assertions(global.failedRequests.count.is(0))
 } 